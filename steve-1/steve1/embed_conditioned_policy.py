...
class MinecraftPolicy(nn.Module):
    ...
    def forward(self, ob, state_in, context):
        ...
        #print("MinecraftPolicy.forward() ob['img'].shape:", ob['img'].shape)
        x = self.img_preprocess(ob["img"])
        #print('MinecraftPolicy.forward() x.shape:', x.shape)
        ...
        #print('MinecraftPolicy.forward() x.shape:', x.shape)
        #t2 = time.time()
        #print(f"MinecraftPolicy.forward() t2-t1:{t2-t1:0.3f}")
...
class MinecraftAgentPolicy(nn.Module):
    ...
    def forward(self, obs, first: th.Tensor, state_in):
        ...
        #print('pi_h:', type(pi_h), pi_h.shape)
        #print("pi_logits['buttons'].shape:", pi_logits['buttons'].shape)
        #print("pi_logits['camera'].shape:", pi_logits['camera'].shape)
        #print('pi_h:', pi_h)
        #print('pi_logits:', pi_logits)
        #t2 = time.time()
        #print(f"MinecraftAgentPolicy.forward() t2-t1:{t2-t1:0.3f}")
    ...
    def act(self, obs, first, state_in, stochastic: bool = True,
        ...
        #t1 = time.time()
        #print("MinecraftAgentPolicy.act() obs['img'].shape:", obs['img'].shape)
        ...
        #print("MineCraftAgentPolicy.act() pd['buttons'].shape:", pd['buttons'].shape)
        if cond_scale is not None:
        ...
        assert not th.isnan(log_prob).any()
        #print("MinecraftAgentPolicy ac['buttons'].shape:", ac['buttons'].shape, ac['buttons'])
        ...
        #print("MinecraftAgentPolicy ac['buttons'].shape:", ac['buttons'].shape, ac['buttons'])
        #t2 = time.time()
        #print(f"MinecraftAgentPolicy.act() t2-t1:{t2-t1:0.3f}")
