...
class InputHandler:
    ...
    def listen_microphone(self, duration: int = 5) -> str:
...
class ActionHandler:
    ...
    def execute_action(self, action: Dict[str, Any], openai_client: OpenAIClient, input_handler: InputHandler, event_store=None):
        ...
        if action_type == "statement":
            ...
            if text:
                ...
                if audio_file:
                    input_handler.play_audio(audio_file)
    ...
    def run(self):
        ...
        try:
            while self.running:
                '''
                # 1. Record audio
                audio_file = self.input_handler.listen_microphone(self.config.audio_interval)
                if not audio_file:
                    time.sleep(1)
                    continue
                ...
                for action in actions:
                    self.action_handler.execute_action(action, self.openai_client, self.input_handler, self.event_store)
                '''
